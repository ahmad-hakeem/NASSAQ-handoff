...
- **Highest-risk code areas:** `backend/routes/auth_routes_mod.py`, `backend/dependencies.py`, `backend/routes/user_roles_routes.py`, `backend/routes/role_dashboards_mod.py`, `backend/routes/parent_portal_routes.py`, `backend/routes/websocket_routes.py`, `backend/routes/monitoring_routes.py`, `backend/routes/school_settings_mod.py`, `backend/routes/principal_management_routes.py`, tenant/auth helpers in `backend/middleware/` and `backend/auth_scope.py`.
 - **Additional scan anchors from 2026-05-14, 2026-05-14 follow-up, and 2026-05-19:** `backend/routes/academics_student_routes.py`, `backend/routes/search_directory_routes_mod.py`, `backend/routes/attendance_routes.py`, `backend/routes/attendance_routes_mod.py`, `backend/routes/reporting_routes_mod.py`, `backend/routes/relationship_routes_mod.py`, `backend/routes/participation_routes_mod.py`, `backend/routes/consent_privacy_routes_mod.py`, `backend/routes/activities_routes_mod.py`, `backend/routes/registration_routes_mod.py`, `backend/routes/independent_teacher_invite_parent_routes.py`, `backend/routes/independent_teacher_invitation_routes.py`, `backend/routes/independent_teacher_workspace_lifecycle_routes.py`, `backend/routes/user_routes_mod.py`, `backend/routes/admin_routes_mod.py`, `backend/routes/audit_routes.py`, `backend/routes/communication_routes.py`, `backend/routes/academics_structure_engine_routes.py`, `backend/routes/academics_year_term_routes.py`, `backend/routes/academics_teacher_routes.py`, `backend/routes/academics_subject_routes.py`, `backend/routes/mfa_routes.py`, `backend/routes/settings_routes.py`, `backend/routes/school_routes_mod.py`, `backend/routes/platform_routes_mod.py`, `backend/routes/security_routes.py`, `backend/engines/audit_engine.py`, `backend/engines/session_engine.py`, and `backend/utils/tokens.py`.
...
- **Usually ignore unless proven reachable:** `backend/scripts/`, test files, migrations, and mockup/sandbox artifacts.
 - **Confirmed weak spots from 2026-05-14 through 2026-05-19:** raw `X-School-Context` and request-parameter/body tenant overrides; legacy academic-structure routes that trust caller-supplied `school_id`; teacher-management and subject-creation routes with missing tenant binding; unified reporting/export endpoints that allow teacher overreach; teacher-reachable directory, relationship, consent/privacy, attendance-alert, and legacy attendance disclosure surfaces; legacy communication read APIs; admin activity-alert telemetry; alternate self role-switch endpoints that bypass hardened MFA/impersonation controls; MFA session revocation paths that fail to revoke the refresh-token family; the legacy `/api/user-roles/return-to-original` restore path; independent-teacher inbox resolution in legacy communication routes (`/communication/received` and `/communication/{message_id}/read`); legacy attendance write/report routes such as `/attendance/record`, `/attendance/mark-all-present`, `/attendance/excuses`, `/attendance/report/summary`, and `/attendance/statistics`; privacy deletion-request / deletion-review workflows in `consent_privacy_routes_mod.py`; platform-admin credential-minting surfaces in `user_routes_mod.py`, `school_routes_mod.py`, and `admin_routes_mod.py` that are reachable without fresh MFA or environment gating; lower-tier school-admin credential and suspension paths in `user_routes_mod.py` and `principal_management_routes.py` that can take over higher-privileged same-tenant accounts; and legacy `PATCH`/`PUT /schools/{school_id}` update paths in `school_routes_mod.py` that let school-role callers tamper with another tenant by supplying a foreign `school_id`. Revisit these first on future scans and treat alternate tenant-context paths, alternate role-switch paths, legacy factory routes, privacy workflow endpoints, credential-minting surfaces, lower-tier admin account-management paths, and legacy by-id school update routes as privileged impersonation or cross-tenant tampering surfaces.
...
The largest project-specific risk is broken access control in multi-role and multi-tenant flows: IDORs in dashboards/portals, unscoped relationship lookups, arbitrary linked-role assignment, cross-tenant context switching, caller-supplied `school_id` parameters, legacy report/directory/attendance/communication endpoints that trust tenant membership alone, same-tenant role-hierarchy breaks where `school_admin` or similar lower-tier roles can reset or suspend higher-privileged accounts, and WebSocket/impersonation behavior that does not match HTTP authorization rules. Platform-admin credential-minting operations are also high-risk: routes that create users, reset passwords, mint school-principal credentials, seed accounts, or otherwise establish durable logins must require the same fresh-MFA posture as other privileged security actions, otherwise a stolen admin bearer token can be turned into a persistent backdoor. The system must enforce least privilege on every route regardless of what the frontend shows or what token claims request, and authorization-changing admin workflows must invalidate stale real-time and switched-session access promptly.

...
12. **Student Enrollment Engine** (12+ ep) — `registration_routes_mod.py` — Teacher registration + **school registration** + student enrollment (create/list/approve/reject), additional info workflow. School approval endpoint `POST /registration-requests/{id}/approve-school` creates school entity + principal user + default settings in one flow.

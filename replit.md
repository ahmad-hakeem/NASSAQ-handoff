...
- **Standby roster manual overrides (Task #102)**: `services/standby_roster_service.py::compute_standby_roster` now reads `standby_overrides` (in `generic_documents`) and merges them on top of the auto distribution (`add` forces a slot, `remove` excludes one). Adds on busy slots are silently dropped at merge time and explicitly rejected with HTTP 409 at the API. Endpoints: `GET /api/standby/roster` (matrix with `status` ∈ standby|busy|blocked|free, `auto`, `override`) and `PUT /api/standby/roster/cell` (action ∈ add|remove|reset; reset deletes any prior override for that exact tuple). Overrides survive timetable regeneration because they live in their own collection. Sibling page `frontend/src/pages/StandbyRosterPage.jsx` mounted at `/school/standby` and `/admin/standby` (entry button added in the Smart Master Grid header). Substitution scoring (`score_candidates_for_slot`) automatically respects overrides because it goes through the same `compute_standby_roster`.
...
- **Schedule teacher-view UX**: in `SchedulePageNew.jsx`, `periodGaps`/`criticalGaps` and the empty-cell `isGap` styling were treating every empty cell in a teacher's view as an "unfilled period", producing a fake "31 من 35 خانة فارغة" warning even though teachers legitimately have free periods. Gated `periodGaps` on `viewMode === 'class'` so the warning panel + red empty-cell styling only fire for class views. Header counter also branches: classes show "X من Y حصة ممكنة"; teachers show "X حصة في الأسبوع • N مواد".
- **Teacher attendance "Mark All Present" reverting**: clicking الكل حاضر then 💾 حفظ would show success toast but old statuses (غائب/متأخر/معذور) reappeared on reload. Root cause in `routes/teacher_attendance_routes.py` `/teacher-attendance/bulk`: when `gd_find_one` returned an existing row, the upsert called `gd_update_one(..., {"_id": existing["_id"]}, ...)`. But `gd_find_one` returns the document with key `id` (not `_id` — that key is popped in `gd_insert`). So `existing["_id"]` was `None`, the filter dict had no usable conditions, and `gd_update_one` ended up updating an arbitrary `teacher_attendance` row (its `select(...).limit(1)` with no WHERE clause). Each "update" silently corrupted a different teacher's record, so the user saw their changes vanish/scramble. Fix: `existing_id = existing.get("id") or existing.get("_id")`, then filter `{"id": existing_id}`. Defensive fallback to natural key `{teacher_id, date, school_id}` if no id resolves. Verified: initial save→ saved:3/updated:0; second bulk overwrite→ saved:0/updated:3; GET returns the 3 records with the new status+check_in_time and total stays at 3 (no duplicates). Note for follow-up: `attendance_routes.py` exposes a parallel teacher-attendance bulk API (different return shape) that should eventually be consolidated, and the upsert isn't protected by a DB-level uniqueness on (teacher_id, date, school_id) — concurrent bulk requests could still race.
...
- **Exam & Assessment Management page** (`/admin/assessments`) — "إدارة الاختبارات والتقييمات" with 3 tabs: Exam Schedule (periods with Arabic ordinal naming, subject CRUD with date/time/class/observer selection, inline period title editing — persisted via `GET/PUT /api/exam-schedule`, single doc per school in `exam_schedule` collection, debounced 400ms autosave with hydration guard), Committees (single + template creation, column-first seating grid, seat cancel/restore, stats/report/copy/import dialogs — persisted via `/api/exam-committees`), Seating Cards (field configuration, dimension controls, print — persisted via `/api/seating-card-settings` with proper find-then-insert/update upsert). Note: `/api/exam-schedule` is intentionally distinct from `/api/exam-periods` which is owned by `academic_structure_routes.py` for academic-year-level exam periods (start_date/end_date/exam_type).
...
8. **التواصل والإشعارات (Communication & Notifications)** → `/teacher/communication` — TeacherCommunicationPage (FULL REWRITE Task #80: Three-section card layout — Communication Center, School Notifications, System Alerts. Communication Center: 6 notification templates (homework/exam/meeting/behavior/achievement/absence) → structured recipient selection (parents individual/all-in-class, admin guidance/general, students multi-select, school staff roles: vice principal, counselor, activity leader, gifted coordinator) → message preview and send. School Notifications: circulars/workshops dropdown filter, card layout. System Alerts: NASSAQ platform alerts card layout. Role-based recipient routing via `recipient_role`. All i18n. Sidebar merged to single "التواصل والإشعارات" item. **April 2026 Wizard refactor**: the multi-view "إرسال إشعار" flow (templates → recipients → parent-mode/staff/admin sub-views → preview) was consolidated into a single-page Progressive Disclosure wizard inside `activeView='wizard'`. Steps render stacked vertically (Step 1 Choose Template, Step 2 Choose Recipients with inline sub-pickers for parents/staff/admin, Step 3 Preview & Send), with framer-motion `AnimatePresence` height/opacity expand animations, auto-scroll into view on each reveal, opacity-60 deemphasis on locked previous steps, brand-turquoise ring + checkmark badge on selected cards, and a sticky clickable progress bar that jumps back to a previous step. Intermediate "Back" links removed; `handleSendMessage` payload preserved exactly. Added dependency: `framer-motion@^11` in `frontend/package.json`.)

...
from src.common.utils.avatar_serving import signed_image_url, attach_image_access_cookie
...
async def login(credentials: UserLogin, request: Request, background_tasks: BackgroundTasks, response: Response):
    from src.core.middleware.rate_limiter import rate_store, rate_limit_headers, identity_tag
    ...
    if acc_limited:
        ...
        from src.common.utils.trusted_proxy import extract_client_ip as _xip
        try:
    ...
    try:
        from src.core.guards.tenant_guard import independent_workspace_id as _it_ws_id
...
async def refresh_token(body: RefreshTokenRequest, request: Request, response: Response):
    ...
    from src.core.middleware.rate_limiter import rate_store as _rl_store, rate_limit_headers
    from src.common.utils.trusted_proxy import extract_client_ip as _xip
...
async def get_me(request: Request, response: Response, current_user: dict = Depends(get_current_user)):
    ...
    from src.core.middleware.rate_limiter import rate_store as _rl_store
    from src.common.utils.trusted_proxy import extract_client_ip as _xip
...
async def get_my_permissions(current_user: dict = Depends(get_current_user)):
    ...
    from src.core.middleware.rbac import RBACMiddleware, ROLE_PERMISSIONS
...
async def set_active_role_context(
    ...
    try:
        ...
        from src.common.utils.trusted_proxy import extract_client_ip as _ip
        from sqlalchemy import text as _sa_text
        await db.session.execute(
            _sa_text(
                """
...
# module's call sites.
from src.common.utils.tokens import token_hash as _token_hash  # noqa: E402
...
async def reset_password(request: ResetPasswordRequest):
    ...
    # password resets.
    from src.core.middleware.rate_limiter import rate_limit_headers
...
async def switch_role(
    ...
    if current_role == UserRole.PLATFORM_ADMIN.value:
        if target_school_id:
            ...
            from src.common.utils.platform_admin_preview import assert_principal_preview_allowed

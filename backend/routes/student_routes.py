...
from services.auth_service import get_current_user, require_roles, hash_password, generate_secure_password
from models.enums import UserRole
import logging

logger = logging.getLogger("nassaq.student_routes")
...
async def find_or_create_parent(db, parent_data: dict, school_id: str, created_by: str):
    """
    ...
    parent_doc = {
        ...
        "created_at": now,
        "created_by": created_by,
        "temp_password": temp_password  # Store temporarily for welcome message
    }
...
def get_db():
    """Get database instance - will be injected from server.py"""
...
@router.post("/create-with-parent")
...
async def check_parent_exists(
    ...
    query = {"school_id": school_id, "$or": []}
    
    if national_id:
        query["$or"].append({"national_id": national_id})
    if phone:
...
@router.post("/bulk-import-with-parents")
async def bulk_import_students_with_parents(
    ...
    for idx, student_data in enumerate(request.students):
        try:
            ...
            # Check for existing parent (sibling detection)
            parent_key = parent_data.get("phone") or parent_data.get("email") or parent_data.get("national_id")
            
            if parent_key in parent_student_map:
                # This is a sibling
                parent_result = parent_student_map[parent_key]
            else:
                parent_result = await find_or_create_parent(
                    db,
                    parent_data,
                ...
                parent_student_map[parent_key] = parent_result
    ...
    for parent_key, parent_result in parent_student_map.items():
        parent_id = parent_result["parent"].get("id")
        parent_doc = await db.parents.find_one({"id": parent_id}, {"_id": 0, "student_ids": 1})
        if parent_doc and len(parent_doc.get("student_ids", [])) > 1:
            results["sibling_groups_created"] += 1
    
    return {
        "success": results["failed"] == 0,
        "results": results,
        "created_students": created_students[:20]  # Return first 20
    }

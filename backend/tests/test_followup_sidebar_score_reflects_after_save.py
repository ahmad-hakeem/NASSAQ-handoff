"""Reproduction (currently FAILING): a live participation score recorded from the
session sidebar must keep flowing into كشف المتابعة *after* the sheet has been
opened once and the lesson has been saved.
...
Realistic cycle this exercises (the exact app flow):
  ...
  3. Teacher saves the lesson / changes settings -> the frontend flushes the
     ENTIRE ``followupData`` blob back (POST /followup-record). This persists the
     *derived* D1 into ``followup_records.data`` as if it were a manual edit.
...
reflected). Bug: the keep-manual guard in ``build_followup_hydration`` treats the
persisted-derived D1 as an immutable manual override, so the sheet freezes at D1
and the second sidebar point never appears.
...
value; only the sidebar drove the score, yet the sheet stops tracking it.

    ...
    <PortalLayout portalType="parent">
      <div className="p-4 space-y-4 max-w-lg mx-auto" dir={isRTL ? 'rtl' : 'ltr'} data-testid="parent-portal-dashboard">
        ...
        {selectedChild && liveData && (
          <>
            <Card className="bg-gradient-to-br from-brand-navy to-brand-purple text-white border-0 rounded-2xl overflow-hidden shadow-lg shadow-brand-navy/15">
              <CardContent className="p-4">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex-1 min-w-0">
                    <p className="text-white/60 text-xs">{formatHijriDate()}</p>
                    <h1 className="text-lg font-bold mt-0.5 truncate font-cairo">
                  ...
                  <div className="flex gap-2 shrink-0">
                    ...
                    >
                      ...
                      {notifications.length > 0 && (
                        <span className="absolute -top-1 -right-1 w-4.5 h-4.5 rounded-full bg-red-500 text-[10px] font-bold flex items-center justify-center ring-2 ring-brand-navy">
                  ...
                  <div className="flex gap-2 mb-3">
                    <button
                      ...
                      className={`flex-1 py-2 rounded-lg text-xs font-medium transition-all ${
                        notifTab === 'admin'
                          ? 'bg-brand-navy/15 text-brand-navy shadow-sm'
                  ...
                  <div className="space-y-2 max-h-48 overflow-y-auto">
                    {(notifTab === 'admin' ? adminNotifs : teacherNotifs).length === 0 ? (
                      ...
                      (notifTab === 'admin' ? adminNotifs : teacherNotifs).map((n, i) => (
                        <div key={n.id || i} className="p-3 rounded-xl bg-gray-50 hover:bg-brand-navy/5 transition-colors">
            ...
            <div className="grid grid-cols-2 gap-3 pb-4">
              <Link to="/parent/communication">
                <Card className="rounded-xl border-0 shadow-sm hover:shadow-md transition-all cursor-pointer group">
                  <CardContent className="p-4 text-center">
                    <div className="w-12 h-12 rounded-xl bg-brand-navy/5 group-hover:bg-brand-navy/10 transition-colors flex items-center justify-center mx-auto mb-2">
              ...
              {selectedChildId && (
                <Link to={`/parent/child/${selectedChildId}/profile`}>
                  <Card className="rounded-xl border-0 shadow-sm hover:shadow-md transition-all cursor-pointer group">
                    <CardContent className="p-4 text-center">
                      <div className="w-12 h-12 rounded-xl bg-brand-purple/5 group-hover:bg-brand-purple/10 transition-colors flex items-center justify-center mx-auto mb-2">

...
export function useStudentProfile() {
  ...
  const generatePlan = async (planType) => {
    ...
    } catch (e) { console.error('Error generating plan:', e); nassaqError(isRTL ? 'فشل في توليد الخطة' : 'Failed to generate plan'); }
    finally { setLoadFn(false); }
  };

  const openExportModal = (planType) => { setExportPlanType(planType || 'both'); setExportFormat('pdf'); setExportModalOpen(true); };
  ...
  const handleSaveActivity = async () => {
    ...
    } catch (e) { console.error('Error saving activity:', e); nassaqError(isRTL ? 'فشل في حفظ النشاط' : 'Failed to save activity'); }
  ...
  const handleDeleteActivity = async (act) => {
    ...
    if (!ok) return;
    try { await api.delete(`/activities/${act.id}`, { headers }); toast.success(isRTL ? 'تم حذف النشاط' : 'Activity deleted'); fetchActivities(); }
    catch (e) { console.error('Error deleting activity:', e); nassaqError(isRTL ? 'فشل في حذف النشاط' : 'Failed to delete activity'); }
  };
  ...
  const handleSaveCertificate = async () => {
    ...
    } catch (e) { console.error('Error saving certificate:', e); nassaqError(isRTL ? 'فشل في حفظ الشهادة' : 'Failed to save certificate'); }
  ...
  const handleDeleteCertificate = async (cert) => {
    ...
    if (!ok) return;
    try { await api.delete(`/activities/certificates/${cert.id}`, { headers }); toast.success(isRTL ? 'تم حذف الشهادة' : 'Certificate deleted'); fetchActivities(); }
    catch (e) { console.error('Error deleting certificate:', e); nassaqError(isRTL ? 'فشل في حذف الشهادة' : 'Failed to delete certificate'); }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (Object.keys(updateData).length === 0) { nassaqWarning(isRTL ? 'لا توجد تغييرات للحفظ' : 'No changes to save'); setSaving(false); return; }
  ...
  const handleAction = async (action) => {
    ...
    try {
      switch (action) {
        case 'suspend': await api.put(`/principal/student/${student.id}/status`, { status: 'suspended' }, { headers }); toast.success(isRTL ? 'تم تعليق الحساب' : 'Account suspended'); break;
        case 'activate': await api.put(`/principal/student/${student.id}/status`, { status: 'active' }, { headers }); toast.success(isRTL ? 'تم تفعيل الحساب' : 'Account activated'); break;
        case 'reset-password': {
        ...
        case 'delete':
          nassaqConfirm(isRTL ? 'هل أنت متأكد من حذف هذا الحساب؟ لا يمكن التراجع.' : 'Are you sure you want to delete this account? This cannot be undone.',
            async () => {
              try { await api.delete(`/students/${student.id}`, { headers }); toast.success(isRTL ? 'تم حذف الحساب' : 'Account deleted'); handleBack(); }
              catch (e) { console.error('Error deleting account:', e); nassaqError(isRTL ? 'فشل حذف الحساب' : 'Failed to delete account'); }
  ...
  const handleSaveBehaviour = async () => {
    if (!behaviourForm.title.trim()) { nassaqError(isRTL ? 'يرجى إدخال عنوان السلوك' : 'Please enter a behavior title'); return; }
    if (!behaviourForm.behaviour_type_id) { nassaqError(isRTL ? 'يرجى اختيار نوع السلوك' : 'Please select a behavior type'); return; }
  ...
  const handleDeleteBehaviour = (record) => {
    nassaqConfirm(isRTL ? 'هل أنت متأكد من حذف هذا السجل؟' : 'Are you sure you want to delete this record?',
      async () => {
        try { await api.delete(`/behaviour-records/${record.id}`, { headers }); toast.success(isRTL ? 'تم حذف السجل' : 'Record deleted'); fetchBehaviourRecords(); }
        catch (e) { console.error('Error deleting record:', e); nassaqError(isRTL ? 'فشل حذف السجل' : 'Failed to delete record'); }
      }, { title: isRTL ? 'تأكيد الحذف' : 'Confirm Delete', confirmText: isRTL ? 'نعم، احذف' : 'Yes, Delete', cancelText: isRTL ? 'إلغاء' : 'Cancel' }
    );
  };
  ...
  const handleAddTalent = async (talentValue) => {
    ...
    try { const newTalents = [...currentTalents, talentValue]; await api.put(`/students/${student.id}`, { talents: newTalents }, { headers }); toast.success(isRTL ? 'تمت إضافة الموهبة' : 'Talent added'); fetchStudent(); }
    catch (err) { const msg = err.response?.data?.detail; nassaqError(typeof msg === 'string' ? msg : (isRTL ? 'فشلت الإضافة' : 'Failed to add talent')); }
  ...
  const handleRemoveTalent = async (talentValue) => {
    ...
    setSavingTalent(true);
    try { await api.put(`/students/${student.id}`, { talents: newTalents }, { headers }); toast.success(isRTL ? 'تمت إزالة الموهبة' : 'Talent removed'); fetchStudent(); }
    catch (e) { console.error('Error removing talent:', e); nassaqError(isRTL ? 'فشلت الإزالة' : 'Failed to remove talent'); }
    finally { setSavingTalent(false); }
  };
  ...
  const handleAddCustomTalent = async () => {
    ...
    } catch (err) { const msg = err.response?.data?.detail; nassaqError(typeof msg === 'string' ? msg : (isRTL ? 'فشلت إضافة الموهبة المخصصة' : 'Failed to add custom talent')); }
  ...
  const handleAddCharacterTrait = async (trait) => {
    ...
    try { await api.put(`/students/${student.id}`, { character_traits: [...current, trait.trim()] }, { headers }); toast.success(isRTL ? 'تمت إضافة السمة' : 'Trait added'); fetchStudent(); setNewCharacterTrait(''); }
  ...
  const handleRemoveCharacterTrait = async (trait) => {
    ...
    const current = student.character_traits || [];
    setSavingCharacterTrait(true);
    try { await api.put(`/students/${student.id}`, { character_traits: current.filter(t => t !== trait) }, { headers }); toast.success(isRTL ? 'تمت إزالة السمة' : 'Trait removed'); fetchStudent(); }
  ...
  const getRiskLabel = (c) => { const map = { critical: isRTL ? 'حرج' : 'Critical', high: isRTL ? 'مرتفع' : 'High', medium: isRTL ? 'متوسط' : 'Medium', low: isRTL ? 'منخفض' : 'Low' }; return map[c] || c; };

            ...
            <div className="w-16 h-16 rounded-2xl bg-white/80 dark:bg-gray-800/60 border border-white/50 dark:border-gray-700/50 shadow-sm flex items-center justify-center overflow-hidden flex-shrink-0">
              <img
                ...
                onError={(e) => {
                  ...
                  e.target.parentElement.innerHTML = `<div class="w-10 h-10 rounded-xl bg-gradient-to-br ${cfg.gradient} flex items-center justify-center"><svg class="h-5 w-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z"></path></svg></div>`;
                }}
              />
            </div>
              ...
              {loading && (
                <div className="flex items-center gap-2 mt-3">
                  ...
                  <span className="text-xs text-muted-foreground font-tajawal">
                    {isRTL ? 'حكيم يحلل ويُعد الخطة...' : 'Hakim is preparing the plan...'}
                  </span>
                </div>
              )}
            </div>
...
export default function StudentProfileDialog({ open, onClose, student, classes = [], onRefresh }) {
  ...
  const generatePlan = async (planType) => {
    ...
    try {
      ...
      const label = planType === 'remedial'
        ? (isRTL ? 'الخطة العلاجية' : 'Remedial Plan')
        : (isRTL ? 'الخطة الإثرائية' : 'Enrichment Plan');
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success(isRTL ? 'تم حفظ بيانات الطالب في قاعدة البيانات بنجاح' : 'Student data saved to database successfully');
      setEditing(false);
      onRefresh?.();
    } catch (error) {
  ...
  const handleAction = async (action) => {
    ...
    try {
      switch (action) {
        ...
        case 'delete':
          nassaqConfirm(
            isRTL ? 'هل أنت متأكد من حذف هذا الحساب؟ لا يمكن التراجع عن هذا الإجراء.' : 'Are you sure you want to delete this account? This action cannot be undone.',
            async () => {
              try {
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="px-6 pt-4 pb-6">
          ...
          <TabsContent value="info" className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                <Label className="text-xs text-muted-foreground">{isRTL ? 'الفصل' : 'Class'}</Label>
                {editing ? (
              ...
              <div className="space-y-2">
                ...
                {editing ? (
                  ...
                  <p className="font-medium">{student.gender === 'male' ? (isRTL ? 'ذكر' : 'Male') : student.gender === 'female' ? (isRTL ? 'أنثى' : 'Female') : '-'}</p>
                )}
              </div>
              <div className="space-y-2">
                <Label className="text-xs text-muted-foreground">{isRTL ? 'رقم الهوية' : 'National ID'}</Label>
            ...
            {student.parent_name && (
              <Card className="bg-muted/30">
                <CardContent className="p-4">
                  <h4 className="font-medium text-sm mb-2">{isRTL ? 'بيانات ولي الأمر' : 'Guardian Info'}</h4>
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    <div><span className="text-muted-foreground">{isRTL ? 'الاسم' : 'Name'}:</span> {student.parent_name}</div>
                    <div><span className="text-muted-foreground">{isRTL ? 'الهاتف' : 'Phone'}:</span> {student.parent_phone || '-'}</div>
                  </div>
                </CardContent>
              </Card>
            )}
            ...
            {loadingRisk ? (
                  ...
                  <Card>
                    <CardContent className="p-4">
                      <h4 className="font-medium text-sm mb-3 flex items-center gap-2">
                        ...
                        {isRTL ? 'نقاط الملاحظة' : 'Key Observations'}
                      </h4>
                      <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <div className="relative flex justify-center">
                    <span className="bg-background px-3 text-xs text-brand-purple font-cairo font-medium">
                      {isRTL ? 'خطط حكيم الذكية' : 'Hakim AI Plans'}
                    </span>
                  </div>
                </div>
              ...
              <Card className="p-8 text-center">
                ...
                <p className="text-muted-foreground text-sm">
                  {isRTL ? 'لا تتوفر بيانات تحليلية حالياً. يحتاج النظام لبيانات حضور وتفاعل لإنشاء التحليل.' : 'No analytics available yet. The system needs attendance and interaction data.'}
                </p>
          ...
          <TabsContent value="behavior" className="space-y-4">
            <Card className="p-8 text-center">
              ...
              <p className="text-muted-foreground text-sm mb-2">
                {isRTL ? 'سجل السلوك والانضباط' : 'Behavior & Discipline Record'}
              </p>
              <p className="text-xs text-muted-foreground">
                {isRTL ? 'يتم تحديث هذا القسم تلقائياً من سجلات الحصص والتقييمات' : 'This section is auto-updated from session and evaluation records'}
              </p>
            </Card>
          </TabsContent>

          <TabsContent value="actions" className="space-y-3">
            <h4 className="font-medium text-sm text-muted-foreground mb-2">{isRTL ? 'إجراءات الحساب' : 'Account Actions'}</h4>

      ...
      {showDrafts && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3.5 mt-3.5 pt-3.5 border-t border-amber-200/60 dark:border-amber-800/50">
          {draftSchools.map((draft) => (
            <div
              key={draft.id}
              className="bg-white dark:bg-slate-900 rounded-xl border border-amber-200/90 dark:border-amber-800/80 p-3.5 shadow-2xs flex flex-col justify-between hover:shadow-sm transition-shadow"
            >
              <div>
                <div className="flex items-center justify-between gap-2 mb-1.5">
                  <h4 className="text-xs sm:text-sm font-bold text-slate-900 dark:text-white font-cairo truncate" title={draft.name}>
                    {draft.name || (isRTL ? 'مسودة بدون اسم' : 'Untitled Draft')}
                  </h4>
                  <span className="text-[10px] font-bold px-2 py-0.5 rounded-md bg-amber-50 text-amber-800 dark:bg-amber-950 dark:text-amber-300 border border-amber-200 dark:border-amber-800 shrink-0">
                    {isRTL ? 'مسودة' : 'Draft'}
                  </span>
                </div>
              ...
              <div className="flex items-center gap-2 pt-2.5 border-t border-slate-100 dark:border-slate-800">
                <Button
                  ...
                  className="flex-1 h-8 rounded-lg bg-[#1C3D74] hover:bg-[#152e57] text-white text-xs font-bold gap-1.5 shadow-xs"
                >
                ...
                <Button
                  ...
                  className="h-8 w-8 p-0 rounded-lg text-rose-600 border-rose-200 hover:bg-rose-50 dark:border-rose-900/60 dark:hover:bg-rose-950/40"

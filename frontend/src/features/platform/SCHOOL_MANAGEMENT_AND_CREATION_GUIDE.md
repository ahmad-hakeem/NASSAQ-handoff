...
| المسار (Endpoint) | الطريقة | الوصف |
| :--- | :--- | :--- |
| `GET /api/schools/{id}/detail` | `GET` | إرجاع ملف تفصيلي كامل عن المدرسة يشمل: عدد الطلاب، المعلمين، الفصول، اشتراك المدرسة، نسبة إكمال الإعداد، وسجل النشاط. |
| `POST /api/schools/{id}/credentials` | `POST` | إعادة توليد بيانات دخول جديدة لمدير المدرسة وتوليد كلمة مرور مؤقتة في حال نسيانها. |
| `GET /api/admin/command-center/schools-overview` | `GET` | إحصائيات مركز القيادة الموحد لجميع المدارس (نشطة، موقوفة، قيد الإعداد). |
